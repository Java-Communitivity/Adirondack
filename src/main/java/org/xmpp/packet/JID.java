 ...
 * $RCSfile$
 * $Revision: 6565 $
 * $Date: 2007-01-04 19:25:02 -0500 (Thu, 04 Jan 2007) $
 *
...
public class JID implements Comparable, Serializable {
    ...
    public static String resourceprep(String resource)  {
        ...
        if (!stringprepCache.containsKey(resource)) {
            // The following line a little bit of a hack, need to makre sure that
            // it meets stringprep requirements. Would love to use stringprep from
            // jivesoftware, but it's not in a Maven repo AFAIK
            answer = IDN.toASCII(resource);
    ...
    private void init(String node, String domain, String resource) {
        ...
        try {
            if (!stringprepCache.containsKey(node)) {
                ...
                //bb this.node = Stringprep.nodeprep(node);
                // Validate field is not greater than 1023 bytes. UTF-8 characters use two bytes.
